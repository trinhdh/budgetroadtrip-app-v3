...
const INITIAL_DAY_DATA = {
    ...
    // Merged Hotel into Timeline for reordering
    timeline: [
        {
            id: '1',
            time: '09:00 AM',
...
export default function DayDetailsScreen() {
    ...
    const saveChanges = () => {
        ...
        } else if (editMode === 'activity' && selectedItemId) {
            setTimelineData(prev => prev.map(item =>
                item.id === selectedItemId
                    ? { ...item, title: editForm.title, desc: editForm.desc, price: Number(editForm.price) || 0 }
                    : item
            ));
        }
        setEditMode('none');
                    ...
                    <View style={styles.activityContent}>
                        <View style={styles.activityHeader}>
                            ...
                            <TouchableOpacity onPress={() => openActivityModal(item)} style={styles.editIconBtn}>
                                <IconSymbol name="edit" size={18} color={colors.icon} />
                            </TouchableOpacity>
                        </View>
    ...
    // --- FOOTER COMPONENT (Add Buttons) ---
    const renderFooter = () => (
        <View style={styles.footerContainer}>
            {/* Add Activity Button */}
            <TouchableOpacity style={[styles.addItemButton, { borderColor: colors.icon + '40' }]}>
            ...
            {dayData.notes ? (
                ...
                <TouchableOpacity
                    onPress={openNoteModal}
                    style={[styles.addItemButton, { borderColor: colors.icon + '40', marginTop: 0 }]}
                >
                    <IconSymbol name="plus" size={20} color={colors.text} />
                            ...
                            <View style={[styles.modalContent, { backgroundColor: colors.background }]}>
                                <View style={styles.modalHeader}>
                                    <ThemedText type="subtitle">
                                        {editMode === 'activity' ? 'Edit Activity' : 'General Note'}
                                    </ThemedText>
                                ...
                                {editMode === 'activity' && (
                                    <>
                                        <View style={styles.inputContainer}>
                                            ...
                                            <TextInput style={[styles.input, { color: colors.text, borderColor: colors.icon }]} value={editForm.title} onChangeText={(text) => setEditForm({ ...editForm, title: text })} />
                                        </View>
                                        <View style={styles.inputContainer}>
                                            <ThemedText style={styles.label}>Description</ThemedText>
                                            <TextInput style={[styles.input, { color: colors.text, borderColor: colors.icon }]} value={editForm.desc} onChangeText={(text) => setEditForm({ ...editForm, desc: text })} />
                                        </View>
                                        <View style={styles.inputContainer}>
                                ...
                                {editMode === 'note' && (
                                    <View style={styles.inputContainer}>
                                        <TextInput style={[styles.textArea, { color: colors.text, borderColor: colors.icon }]} value={editForm.notes} onChangeText={(text) => setEditForm({ ...editForm, notes: text })} placeholder="Add notes..." multiline numberOfLines={6} textAlignVertical="top" autoFocus={true} />
                                    </View>
                                )}
